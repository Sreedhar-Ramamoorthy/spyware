...
import org.rifluxyss.javadev.harddrivescanner.commons.gui.tablemodels.AdvancedFileListTableModel;

public class AdvancedFileListPanel extends JPanel{

    static int lastSelectedRow = -1;
    static FileSelectionListener selectionListener; 
	public AdvancedFileListPanel(JPanel _basicListPanel, int _categoryIndex, int _extensionIndex){
		
		setLayout(null);
		setBackground(Color.WHITE);
		
		ClickListener _clickListener = new ClickListener(_basicListPanel, _categoryIndex, _extensionIndex);
    	...
    	_backButton.setActionCommand("REFRESH_FILE_TYPE");
    	...
    	_refreshButton.setActionCommand("REFRESH_ADVANCED_FILE_LIST");
    	...
    	SpyJLabel _categoryTitleLabel = new SpyJLabel(SpyJUtilities.getExtensionName(_categoryIndex, _extensionIndex).toUpperCase());
		...
		TableModel fileListTableModel = new AdvancedFileListTableModel(_categoryIndex, _extensionIndex);
		...
		*/			
		...
		fileListColumnModel.getColumn(0).setPreferredWidth(120);
		fileListColumnModel.getColumn(1).setPreferredWidth(450);
		fileListColumnModel.getColumn(2).setPreferredWidth(100);
		fileListColumnModel.getColumn(3).setPreferredWidth(75);
		...
		fileListColumnModel.getColumn(3).setCellRenderer(alignRendererNew);
		...
		final TableCellRenderer fileModifiedCellRenderer = fileListColumnModel.getColumn(2).getCellRenderer();
		fileListColumnModel.getColumn(2).setCellRenderer(new TableCellRenderer() {
            @Override
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                JLabel lbl = (JLabel) fileModifiedCellRenderer.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
                lbl.setFont(SpyJGlobals.SansPlain12);
                lbl.setBorder(BorderFactory.createCompoundBorder(lbl.getBorder(), BorderFactory.createEmptyBorder(0, 5, 0, 0)));
                lbl.setHorizontalAlignment(SwingConstants.LEFT);
                return lbl;
		...
		final TableCellRenderer fileSizeCellRenderer = fileListColumnModel.getColumn(3).getCellRenderer();
		fileListColumnModel.getColumn(3).setCellRenderer(new TableCellRenderer() {
            @Override
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
